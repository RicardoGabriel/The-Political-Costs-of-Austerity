* Using a different source of narratively identified fiscal consolidations (IMF)				
...
*create election*region fixed effects
egen ttt = group(id electionyear)
...
foreach lhsvar in `lhsvarlist'   {/*LHS variable: `lhsvarlist' */
	...
	* One regression for each horizon of the response (0-4 years)
	forvalues i = 0/`horizon' {
		...
		* LP regression	
		xi: ivreg2 F`i'`lhsvar' (F0`var'RHS  = IMF) ///
...
tex (7) IMF austerity shock \$ s_{it} \$ & ${b_0}${star_0} & ${b_1}${star_1}  & ${b_2}${star_2} & ${b_3}${star_3} & ${b_4}${star_4}  \\
