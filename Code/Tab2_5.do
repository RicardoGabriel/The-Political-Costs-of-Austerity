* Election*Country Fixed Effects				
...
*create election year dummy
gen electionyear = (Far_Left!=l.Far_Left)
replace electionyear = Year if electionyear == 1
replace electionyear = 1979 if electionyear == 0

*create country*election fixed effects
egen ttt = group(cid electionyear)
	...
	* One regression for each horizon of the response (0-4 years)
	forvalues i = 0/`horizon' {
		...
		* LP regression	
		...
		`rhscontrols`lhsvar'' i.ttt,  ///
		cluster(id) partial(`rhscontrols`lhsvar'' i.ttt)
	
		
		* for elction*region fixed effects
		*ivreghdfe F`i'`lhsvar' (F0`var'RHS  = `inst') `rhscontrols`lhsvar'', absorb(ttt) cluster(id)
...
tex (5) Including country-election FE & ${b_0}${star_0} & ${b_1}${star_1}  & ${b_2}${star_2} & ${b_3}${star_3} & ${b_4}${star_4}  \\
