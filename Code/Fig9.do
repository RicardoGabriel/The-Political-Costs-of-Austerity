*Difference in responses between austerity-recessions and non-austerity recessions


use "${hp}Data\Out\Data_Final_nuts${nu}.dta", clear
merge 1:1 Nuts_id Year using "${hp}\Data\Algan\Trust_Algan.dta"
drop if _merge == 2
sort id Year
...
***********************Baseline IV LP graphs**********************************
	...
	** LPs
	if "`lhsvar'" == "Far_share" {
		...
		graph export "$Fig\Fig9a.eps", replace
		...
		graph export "$Fig\Fig9b.eps", replace
		...
		graph export "$Fig\Fig9c.eps", replace
		...
		graph export "$Fig\Fig9d.eps", replace
