* Unpredicted Austerity (takes into account possible anticipation effects by using as the instrument the residuals from
* regressing the austerity shock on a set of macroeconomic variables, including two lags of output and consumption
* growth, debt-to-GDP ratio, and real short- and long-term interest rates)
...
foreach lhsvar in `lhsvarlist'   {/*LHS variable: `lhsvarlist' */
	...
	* One regression for each horizon of the response (0-4 years)
	forvalues i = 0/`horizon' {
		...
		* LP regression	
		xi: ivreg2 F`i'`lhsvar' (F0`var'RHS  = Alesina4_unpred) ///
...
tex (2) Unpredicted austerity & ${b_0}${star_0} & ${b_1}${star_1}  & ${b_2}${star_2} & ${b_3}${star_3} & ${b_4}${star_4}  \\
